...
export class QuizQuestionRepoService {
  ...
  public async Create(
    inputData: QuizQuestionPostEntity,
    published: boolean = false,
  ) {
    let quizQuestDto = QuizQuestionEntity.Init(
      inputData.body,
      inputData.correctAnswers,
      published,
    );
  ...
  public async CountAndReadManyByName(
    ...
    if (bodyPattern)
      BodySearchPattern['body'] = Raw((alias) =>
        caseInsensitiveSearchPattern(alias, bodyPattern),
      );
